...
const TransactionConfirm: FC<any> = ({ setCurrentStep, handleSubmit, data = {} }) => {
  ...
  const getIcon = () => {
    ...
    return data.token?.ticker === "BTC" && data.token?.id === undefined  ? <Bitcoin /> : GetIconLogo(data.token?.ticker, data.token?.id);
  };
  ...
  const getTotalAmount = (data: any) => {
    ...
    if (data.token?.ticker === "BTC" && data.token?.id === undefined) {
      ...
      if (data.token?.amt === amountParsed) {
        return setTotal(amountParsed);
      }

      return setTotal(parseFloat(totalSats.toFixed(8)));
    }
  ...
  return (
    <div> {/* This div becomes the relative container for the overlay */}
      {isSubmitting && (
      ...
      <div className="content">
          ...
          <div className="flex items-center">
            ...
            <div className="flex flex-col ml-3">
              <p className="text-base text-white">
                {data.token?.ticker === "BTC" && data.token?.id === undefined ? 'BTC': `${data.token?.ticker}:${data.token?.id}`}
              </p>
              <p className="text-gray text-sm">
                {data.amount}
              </p>
        ...
        <div className="bg-dark-gray py-3 px-6 rounded-md">
          ...
          <p className="text-white text-[28px] font-bold my-3">
            {total}
          </p>
          <p className="text-gray text-sm">
            {`${data.calculatedFee.toFixed(8)} BTC (network fee)`}
          </p>
        </div>
